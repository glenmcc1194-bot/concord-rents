...
export async function upsertEquipment(item) {
  // Base edits on the DB list once it exists; before the first save, start from the
  // full seed inventory so adding/editing one machine doesn't wipe the other 37.
  let list;
  if (kvConfigured()) {
    const raw = await redis(['GET', 'equipment']);
    list = raw ? JSON.parse(raw) : [...(await seedInventory())];
  } else {
    list = [...(await seedInventory())];
  }
